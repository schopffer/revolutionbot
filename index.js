...
const membreRoleId = '1385627871023861820'; // rôle qui peut utiliser /blague
...
client.once('ready', async () => {
  ...
  const commands = [
    ...
    new SlashCommandBuilder().setName('mute').setDescription('Rendre un membre muet').addUserOption(opt =>
      opt.setName('membre').setDescription('Membre à rendre muet').setRequired(true)),
    new SlashCommandBuilder().setName('unban').setDescription('Débannir un membre').addStringOption(opt =>
      opt.setName('userid').setDescription("ID du membre à débannir").setRequired(true)),
...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (interaction.isCommand()) {
    ...
    // ... ici tu peux ajouter le reste des commandes (ban, kick, mute, etc.) avec tes vérifications de permissions
...
**👑 Staff** : respect des décisions.
    ...
    // Ajoute ici les autres commandes admin (ban, kick, mute, unban) avec leurs vérifications

  }

  // BOUTON valider règlement
...
// Gestion des rôles par réactions emoji dans choixRoleChannelId
...
// Logs embeds améliorés (ban, unban, message supprimé, édité, etc.)
